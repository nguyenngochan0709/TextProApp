...
public class EfficientDocument extends Document {
	...
	private void processText()
	{
		...
		String[] arr_tokens = tokens.toArray(new String[tokens.size()]);
		for (int i = 0; i < arr_tokens.length; i++) {
			if (isWord(arr_tokens[i])) {
				numWords += 1;
				numSyllables += countSyllables(arr_tokens[i]);
				if (i == (arr_tokens.length-1)) {
					numSentences += 1;
				}
			} else {
				numSentences += 1; 
